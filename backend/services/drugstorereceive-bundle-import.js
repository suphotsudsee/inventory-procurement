...
function readCsvContent(content, expectedHeader, fileName) {
  const normalized = String(content || '').replace(/^\uFEFF/, '').trim();
  if (!normalized) {
    throw new Error(`CSV content is empty: ${fileName}`);
  }

  const lines = normalized.split(/\r?\n/);
  const header = parseCsvLine(lines[0]);
  if (JSON.stringify(header) !== JSON.stringify(expectedHeader)) {
    throw new Error(`Unexpected CSV header in ${fileName}: ${header.join(', ')}`);
  }
...
async function importDrugstoreReceiveBundle({
  ...
  const receiveRows = readCsvContent(
    receiveContent,
    ['pcucode', 'receiveno', 'receivedate', 'receitype', 'recievenoreal', 'companyname', 'remark'],
    receiveSourceRef
  );
  const detailRows = readCsvContent(
    detailContent,
    ['pcucode', 'receiveno', 'drugcode', 'amount', 'cost', 'expiredate', 'lotno', 'remark'],
    detailSourceRef
  );
